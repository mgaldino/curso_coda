...
Essa é o tutorial para o curso de R para análise de políticas públicas.
A oficina prática ensinará como utilizar o software R para fazer análises estatísticas e manuseio de microdados de políticas públicas. Iremos aprender a importar dados simples e as principais bases de dados de microdados do Brasil, e fazer análises básicas delas. Ao final da oficina, o participante saberá as principais funções para usar o R em análises de políticas públicas.
Ao longo do dia iremos:
    ...
    6. Aprender a fazer gráficos com ggplot2
    7. Trabalhar problemas do mundo real na parte 2 e conhecer algumas bases de dados mais relevantes (microdados).


## Primeiros passos

O curso tem bastante similaridade com o curso de R que dei no congresso da ABraji.
...
Sempre verifique se você digitou *corretamente* as funções no R. Muitos dos problemas enfrentados pelos usuários são causados por uma vírgula fora do lugar.
...
######## Curso de R análise de políticas públicas
...
E podemos criar nós mesmos os nossos objetos utilizando o operador de atribuição <- 
Esses objetos ficarão disponíveis no *Global Environment* e, assim, poderemos utilizá-los depois.
...
Se não der nenhuma mensagem de erro, é porque deu certo.
...
É uma prática comum indicar as bibliotecas que serão usadas no início do script, no entanto, a longo do curso de hoje faremos um pouco diferente: sempre que utilizarmos um comando que pertence a uma biblioteca, vamos mostrar a biblioteca como um comentário. Vejam o exemplo a seguir (por favor, não rodem):
...
# Definindo o caminho
setwd("C:/Users/mgaldino/2018/Pessoal/curso_coda")
...
fifa2018 <- read_excel("C:/Users/mgaldino/2018/Pessoal/curso_coda/fifa2018.xlsx", 
...
Essa média ficou com muitas casas decimais, não acham? 
Podemos controlar o número de casas decimais (no R representados pelo .) utilizando o *round()*, que arredonda números:
...
Mais limpo, não acham? E como não fechamos o parenteses do *mutate()*, não precisams colocar o pipe para continuar com o código.

###Renomeando variáveis:

Para renomear variáveis, usamos o *rename()* do pacote dplyr, indicando primeiro o nome novo e depois o nome antigo.

Importante: o R tem dificuldade em ler nomes com espaços e acentuação. Por isso, é uma boa prática sempre deixar todas as letras das variáveis como minúsculas para evitar erros na hora de digitar o código.
...
Agora que trabalhamos bastante com o objeto "fifa2018_1", está na hora de salva-lo. Vamos aprender como exportar um banco como um .csv.
...
Uma das fortalezas do R é a parte de visualização gráfica, especialmente como pacote ggplot2. Vamos fazer uns gráficos?

O ggplot2 é muito poderoso. Vamos começar entendedo como ele funciona.
Digamos que eu quero fazer um gráfico simples entre altura e gols. A sintaxe começa assim:

```{r, eval=FALSE, message=FALSE, warning=FALSE}
library(ggplot2)
# ggplot(banco_de_dados, aes(x, y))
```
...
```{r, eval=TRUE, message=FALSE, warning=FALSE}
library(ggplot2)
ggplot(fifa2018_1, aes(goals, height))
```


Um plot vazio é renderizado. Nós especificamos o que são x e y, não temos pontos, nem retas. Para o ggplot, se você não especificar qual é o formato do gráfico (pontos, linhas, barras etc.), ele não saberá o que você quer renderizar. Eu apenas falei qual banco usar, e quais variáveis são ser o x e y.

E a função aes() é usada para especificar os eixos x e y.

Para plotar pontos (por exemplo), preciso dizer que essa é a geometria que eu quero.

```{r, eval=TRUE, message=FALSE, warning=FALSE}
#library(ggplot2)
ggplot(fifa2018_1, aes(goals, height)) + geom_point()
```

Agora temos um gráfico plotado. Cada ponto representa um jogador (altura e gools marcados). Muitas coisas podem ser customizadas nesse gráfico, como o nome das variáveis, nome do título de plot, cor de background, tamanho das fontes etc.

E temos muitas outras geometrias possíveis: geom_bar para gráfico de barra, geom_line para gráfico de linha e assim por diante.

De todo modo, vocês concordam que não há correlação entre altura e gols? Vamos traçar uma linha reta associado as duas variáveis, para visuzalizar melhor?


```{r, eval=TRUE, message=FALSE, warning=FALSE}
#library(ggplot2)
ggplot(fifa2018_1, aes(goals, height)) + geom_point() +geom_smooth(method="lm")
```

Eu usei uma nova geometria, de alizamento (smooth), que plota um ajuste aos dados. No caso, um modelo liear (por isso method = "lm", de linear model). Vejam quea correlação estimada é levemente negativa, mas ele também plotou um "intervalo de confiança", e aí vemos que a correlação é próxima de zero. Se você quiser o plot sem os intervalos, basta colocar "se=FALSE" dentro de geom_smooth (separado por vírgula, pois é outro argumento) para remover os intervalos. De todo modo, será que se olharmos só para atacantes, a associação muda?
...
fifa2018_1  %>%
  filter(position == "FW") %>%
  ggplot(aes(goals, height)) + geom_point() + geom_smooth(method="lm")
```

Vejam que usei o filter para filtrar e, com o pipe, não precisei especificar qual o banco de dados. Ficou claro que só olhando para atacante, não tem correlaçção nenhuma.

Vamos só deixar o gráfico mais bonito? Vamos mudar os nomes das variáveiss e adicionar um título ao gráfico?

```{r, eval=TRUE, message=FALSE, warning=FALSE}
#library(ggplot)
fifa2018_1  %>%
  filter(position == "FW") %>%
  ggplot(aes(goals, height)) + geom_point() + geom_smooth(method="lm") + xlab("Gols feitos") + ylab("Altura em cm") + ggtitle("Associação entre gols marcados por atacantes \n e altura dos jogadores das seleções de futebol", subtitle="Dados oficiais da Fifa")
```

xlab e ylab alteram o nome das variáveis x e y. E ggtitle altera o título do gráfico (e subtítulo). O 
```{r, eval=FALSE, message=FALSE, warning=FALSE}
"\n"
``` 
indica quebra de linha.

E seu eu quiser mudar outras coisa,s podemos usar um tema do ggplot. Por exemplo:

```{r, eval=TRUE, message=FALSE, warning=FALSE}
#library(ggplot)
fifa2018_1  %>%
  filter(position == "FW") %>%
  ggplot(aes(goals, height)) + geom_point() + geom_smooth(method="lm") + theme_bw() + xlab("Gols feitos") + ylab("Altura em cm") + ggtitle("Associação entre gols marcados por atacantes \n e altura dos jogadores das seleções de futebol", subtitle="Dados oficiais da Fifa")
```
Com essa introdução a como operar com R, podemos agora mexer em dados mais úteis para análises de políticas públicas com R do que os dados da Fifa.

Abra o guia da segunda parte da aula aqui:
